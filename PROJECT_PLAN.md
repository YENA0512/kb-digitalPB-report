...
### [D] Noti View (알림 및 맞춤 설정)
*   **스마트 알림 시스템:** 
    *   사용자가 특정 지수(KOSPI, S&P 500 등)나 보유 종목의 목표가/손절가를 설정 가능.
    *   설정된 임계값 도달 시 실시간 알람 생성 및 히스토리 관리.
*   **AI 맞춤 알림:** 시장 급변동, 뉴스 기반 자산 변동성 감지 시 AI가 먼저 알림을 보냄.

### [E] Global Interactions & Components
*   **상세 추천 모달 (AI Pick Detail):**
    *   추천 상품 클릭 시 등장. 추천 사유, AI 점수, 예상 수익률 범위를 상세 시각화.
    *   '즉시 구매' 클릭 시 자산 테이블에 시뮬레이션 반영.
*   **시뮬레이션 모달:**
...
- `/src/App.tsx`: 메인 애플리케이션, 라우팅 및 알림/상세 모달 로직
...
5.  **기능 고도화:** 리밸런싱 시뮬레이션, 스마트 알림, 인터랙션 디테일(Framer Motion) 강화
